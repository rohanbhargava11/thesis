...
d=c.height()>f.height()?16:0;e=this.options.isSortable&&i&&parseInt(jQuery.browser.version,10)<8?d:0;c.parent().parent().find("table.ui-ncbigrid-header").width(c.width()+2-e);c=f.width()-d;g.width(c);this.parts.captionDiv&&this.parts.captionDiv.width(c);b.each(a)},_handleScroll:function(a,b,c){function d(){g._handleScroll(a,b,true)}var f=a.scrollLeft;b.scrollLeft=f;this.parts.captionDiv&&this.parts.captionDiv.css("margin-left",f+"px");var g=this;if(!c){window.setTimeout(d,1);window.setTimeout(d,5)}},
...
function(){c.element!=this&&a(this).ncbitoggler("updateState",b)})}})})(jQuery);
...
this.element.wrapInner("<div class='expanderWrapper'></div>").find(".expanderWrapper")},_setHeightAndClass:function(){this._wrapper.addClass("ui-ncbiexpander").height(this.options.minHeight);(this.isExpandable=this._wrapper[0].scrollHeight>this._wrapper.height())||this._wrapper.addClass("ui-ncbiexpander-simple").height("auto")},_hookUpEvents:function(){var a=this,b;if(this.isExpandable){var c=a._wrapper[0].scrollHeight;if(this.options.auto==true){b={interval:this.options.openDelay,timeout:this.options.delayTimeout,
over:function(){a._isOpen=true;a._wrapper.stop().animate({height:c},a.options.animationSpeed,function(){a._setArrowDirection("open")})},out:function(){a._isOpen=false;a._wrapper.stop().animate({height:a.options.minHeight},a.options.animationSpeed,function(){a._setArrowDirection("closed")})}};this.element.hoverIntent(b);this.element.mouseenter(function(){a._setIconBarStyle(true)}).mouseleave(function(){a._isOpen||a._setIconBarStyle(false)})}else a._attachControlLinkEvents(a,c)}},_attachControlLinkEvents:function(a,
b){this.link=jQuery(this.element).find(".ui-expander-moreless");jQuery(this.link).on("click",function(c){c.preventDefault();if(a._isOpen==false){a._isOpen=true;a._wrapper.stop().animate({height:b},a.options.animationSpeed);jQuery(this).text("...less").removeClass("ui-expander-more").addClass("ui-expander-less")}else{a._isOpen=false;a._wrapper.stop().animate({height:a.options.minHeight},a.options.animationSpeed);jQuery(this).text("...more").removeClass("ui-expander-less").addClass("ui-expander-more")}})},
_addNavigator:function(){this.options.auto==false&&this.isExpandable?this._addControlLink():this._addArrow()},_addControlLink:function(){this._isOpen=false;this._iconBar=jQuery('<span class="ui-expander-controllink ui-helper-clearfix"><a href="" class="controlanchor ui-expander-more ui-expander-moreless">...more</a></span>').appendTo(this.element)},_addArrow:function(){this._iconBar=jQuery('<div class="ui-expander-arrow-bar"><div class="bar closed"><div class=""></div></div></div>').appendTo(this.element)},
_setArrowDirection:function(a){var b=a==="open"?"closed":"open";this._iconBar.find("div.bar").addClass(a).removeClass(b);this._setIconBarStyle(a==="open")},_setIconBarStyle:function(a){this._iconBar[a?"addClass":"removeClass"]("ui-expander-arrow-bar-active")}});
...
this.selectItems.eq(a).height();a=this.selectItems.eq(a).offset().top-c+f;d<a&&b.prop("scrollTop",a-d)}},getAllSelectedIndexes:function(){var a=[];this.element.find("option:selected").each(function(b){a.push(b)});return a},getGeneratedElem:function(){return this._ul}});
...
a;if(this.options.errorBorder)if(a=="valid")this.element.removeClass("ncbiactivevalidation_el_error");else a=="error"&&this.element.addClass("ncbiactivevalidation_el_error")},getValidationStatus:function(){return this._last||null}});
